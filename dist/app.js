...
const processBtn=$('#process-all'), facebookBtn=$('#facebook-all'), shareBtn=$('#share-all'), saveBtn=$('#save-all');
const logo=new Image();
const logoReady=fetch('./assets/dm-creations-watermark.b64').then(r=>{if(!r.ok)throw new Error('Watermark kon niet worden geladen.');return r.text()}).then(b64=>new Promise((resolve,reject)=>{logo.onload=resolve;logo.onerror=reject;logo.src='data:image/png;base64,'+b64.trim()}));
...
function changed(){for(const item of items){item.output=null;item.card.querySelector('.item-actions').hidden=true}facebookBtn.disabled=shareBtn.disabled=saveBtn.disabled=true;statusEl.textContent=items.length?'Voorbeelden bijgewerkt. Tik op Alles verwerken voor delen of bewaren.':'';clearTimeout(renderTimer);renderTimer=setTimeout(renderAllPreviews,90)}
...
async function renderPreview(item){const loading=item.card.querySelector('.loading'),err=item.card.querySelector('.item-error');loading.hidden=false;err.hidden=true;try{await logoReady;if(!item.bitmap)item.bitmap=await loadBitmap(item.file);draw(item.canvas,item.bitmap,1400);item.card.querySelector('.file-meta').textContent=`${item.bitmap.width} × ${item.bitmap.height} · ${friendlyBytes(item.file.size)}`}catch(e){err.textContent=e.message;err.hidden=false}finally{loading.hidden=true}}
async function renderAllPreviews(){await logoReady.catch(()=>{});await Promise.all(items.map(renderPreview))}

input.addEventListener('change',async()=>{for(const file of input.files){const t=$('#preview-template').content.cloneNode(true),card=t.querySelector('.preview-card'),item={file,card,canvas:t.querySelector('canvas'),bitmap:null,output:null};card.querySelector('.file-name').textContent=file.name;card.querySelector('.remove').addEventListener('click',()=>removeItem(item));card.querySelector('.facebook-one').addEventListener('click',()=>shareToFacebook([item]));card.querySelector('.share-one').addEventListener('click',()=>shareFiles([item]));card.querySelector('.save-one').addEventListener('click',()=>saveToPhotos([item]));list.append(t);items.push(item)}input.value='';refresh();await renderAllPreviews()});
function removeItem(item){if(item.bitmap?.close)item.bitmap.close();items=items.filter(x=>x!==item);item.card.remove();refresh()}
function refresh(){empty.hidden=items.length>0;processBtn.disabled=!items.length;$('#preview-help').textContent=items.length?`${items.length} foto${items.length===1?'':'’s'} klaar voor verwerking.`:'Kies foto’s om het resultaat vooraf te bekijken.';if(!items.length){facebookBtn.disabled=shareBtn.disabled=saveBtn.disabled=true;statusEl.textContent=''}}
...
async function shareFiles(selected){try{const files=await ensureOutputs(selected);if(!files.length)return;if(await nativeShare(files,'DM-Creations foto’s','DM-Creations')){statusEl.textContent='Deelmenu geopend.'}else if(files.length>1){statusEl.textContent='Je apparaat kan deze foto’s niet samen delen. Gebruik Delen bij elke foto.';selected.forEach(i=>i.card.querySelector('.item-actions').hidden=false);selected[0].card.scrollIntoView({behavior:'smooth',block:'center'})}else{statusEl.textContent='Delen van bestanden wordt hier niet ondersteund.'}}catch(e){if(e.name!=='AbortError')statusEl.textContent='Delen lukte niet. Probeer opnieuw.'}}
async function shareToFacebook(selected){try{const files=await ensureOutputs(selected);if(!files.length)return;if(await nativeShare(files,'DM-Creations','Nieuwe DM-Creations kaart')){statusEl.textContent='Kies Facebook in het deelmenu. Facebook bepaalt daarna waar je kunt plaatsen.'}else{statusEl.textContent='Facebook delen werkt op dit apparaat via het gewone deelmenu. Gebruik Delen.'}}catch(e){if(e.name!=='AbortError')statusEl.textContent='Facebook delen lukte niet. Probeer Delen.'}}
async function saveToPhotos(selected){try{const files=await ensureOutputs(selected);if(!files.length)return;if(await nativeShare(files,'DM-Creations foto’s','')){statusEl.textContent='Kies in het deelmenu “Bewaar afbeelding” of “Bewaar afbeeldingen”.'}else{files.forEach((f,i)=>setTimeout(()=>saveFile(f),i*350));statusEl.textContent='Download gestart. Safari kan niet rechtstreeks in Foto’s opslaan.'}}catch(e){if(e.name!=='AbortError')statusEl.textContent='Bewaren lukte niet. Probeer opnieuw.'}}
facebookBtn.addEventListener('click',()=>shareToFacebook(items));
